...
export class AdminController {
  ...
  @Get('public-settings') publicSettings() {
    ...
    return { app_name: settings.app_name || 'yaemipaste', public_title: settings.public_title || 'yaemipaste', registration_enabled: settings.registration_enabled !== 'false', base_api_url: settings.base_api_url || '', file_size_limit_bytes: Number(settings.file_size_limit_bytes || 0), file_size_limit_unlimited: settings.file_size_limit_unlimited === 'true', upload_access_mode: settings.upload_access_mode === 'public' ? 'public' : 'private', turnstile_site_key: settings.turnstile_site_key || '', turnstile_required: settings.turnstile_enabled === 'true' && !!settings.turnstile_site_key?.trim() && turnstileSecretConfigured }
  }
  ...
  @Put('settings') updateSettings(@Req() request: Request, @Body() body: Record<string, any>) { const actor = this.actor(request); const allowed = ['app_name', 'public_title', 'base_api_url', 'registration_enabled', 'file_size_limit_bytes', 'file_size_limit_unlimited', 'upload_access_mode', 'turnstile_enabled', 'turnstile_site_key', 'turnstile_secret_key']; const now = nowSeconds(); for (const key of allowed) if (body[key] !== undefined) { let value = body[key]; if (typeof value === 'boolean') value = value ? 'true' : 'false'; if (key === 'upload_access_mode' && !['private', 'public'].includes(value)) throw apiError(400, 'Invalid upload access mode'); if (key === 'base_api_url' && value && !/^https?:\/\/[^\s:@]+(?:\/[^\s]*)?$/i.test(String(value))) throw apiError(400, 'Base API URL must be http(s) without credentials'); this.db.run('INSERT INTO admin_settings (key,value,updated_at,updated_by) VALUES (?,?,?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at,updated_by=excluded.updated_by', [key, String(value), now, actor.username]) } this.auth.audit(actor.username, 'admin.settings.update', null, 'success'); this.auth.dispatchWebhook('admin.settings.updated', { actor: actor.username, keys: allowed.filter(key => body[key] !== undefined) }); return this.safeSettings() }
