...
export class AdminController {
  ...
  @Put('settings') updateSettings(@Req() request: Request, @Body() body: Record<string, any>) { const actor = this.actor(request); const allowed = ['app_name', 'public_title', 'base_api_url', 'registration_enabled', 'file_size_limit_bytes', 'file_size_limit_unlimited', 'upload_access_mode', 'passkeys_enabled', 'turnstile_enabled', 'turnstile_site_key', 'turnstile_secret_key', 'accent_color', 'logo_type', 'logo_preset', 'branding_logo']; const now = nowSeconds(); for (const key of allowed) if (body[key] !== undefined) { let value = body[key]; if (typeof value === 'boolean') value = value ? 'true' : 'false'; if (key === 'passkeys_enabled' && !['true', 'false'].includes(String(value))) throw apiError(400, 'passkeys_enabled must be a boolean'); if (key === 'upload_access_mode' && !['private', 'public'].includes(value)) throw apiError(400, 'Invalid upload access mode'); if (key === 'base_api_url' && value && !/^https?:\/\/[^\s:@]+(?:\/[^\s]*)?$/i.test(String(value))) throw apiError(400, 'Base API URL must be http(s) without credentials'); if (key === 'accent_color' && value && !/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(String(value))) throw apiError(400, 'Accent color must be a hex value like #fbbf24'); if (key === 'logo_type' && !['', 'preset', 'upload'].includes(String(value))) throw apiError(400, 'Invalid logo type'); if (key === 'logo_preset' && value && !/^[a-z0-9-]{1,32}$/.test(String(value))) throw apiError(400, 'Invalid logo preset'); if (key === 'branding_logo' && value && !/^data:image\/(?:png|jpeg|jpg|gif|webp|x-icon|svg\+xml);base64,/.test(String(value))) throw apiError(400, 'Logo must be a base64 image data URL'); if (key === 'branding_logo' && String(value).length > 1_500_000) throw apiError(400, 'Logo image is too large (max ~1 MB)'); this.db.run('INSERT INTO admin_settings (key,value,updated_at,updated_by) VALUES (?,?,?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at,updated_by=excluded.updated_by', [key, String(value), now, actor.username]) } if (body.passkeys_enabled !== undefined && String(body.passkeys_enabled).toLowerCase() === 'false') this.auth.invalidatePasskeyCeremonies(); this.auth.audit(actor.username, 'admin.settings.update', null, 'success'); this.auth.dispatchWebhook('admin.settings.updated', { actor: actor.username, keys: allowed.filter(key => body[key] !== undefined) }); return this.safeSettings() }
